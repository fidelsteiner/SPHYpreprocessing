...
write.table(glac_table, file = path_maps&'\\glac_table.csv', append = FALSE,col.names = T,row.names = F,sep=',')
##########################

##########################
# Visualize domain data
  png(file=path_output&'\\Domain_Glaciers.png', res = 160,width=dim(dem)[2]*2,height=dim(dem)[1]*2)
  ...
  plot(sub_15,add=T,border='blue')
  plot(sub_15_debris,add=T,border='blue',col='grey')
  legend('bottomright',bty="n",c('glaciers','debris cover'),pch = 1,col = c('blue','grey'))
